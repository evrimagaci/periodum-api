...
const mockedChangeInterval = jest.fn();
const service = new PubChemService({
	changeInterval: mockedChangeInterval,
	interval: 1000
});
...
describe('PubChem API Tests', () => {
	...
	describe('Throttle Control', () => {
		beforeAll(() => {
			...
			jest.requireMock('axios').default.get =
				mockedAPI.getCompoundByIdWithThrottle;
		});

		it('should call changeInterval when x-throttling-control is high in response header', async () => {
			await service.getParsedCompoundById(222);
			expect(mockedChangeInterval).toHaveBeenCalled();
		});
	});
});
...
describe('should parse RawCompound with id 222 properly', () => {
	...
	beforeAll(async () => {
		jest.requireMock('axios').default.get = mockedAPI.getCompoundById;
